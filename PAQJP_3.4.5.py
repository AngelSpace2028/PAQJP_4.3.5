...
class SmartCompressor:
    ...
    def transform_07(self, data, repeat=100):
        """
        Transformation using 3 base-10 digits of pi, mapped to 0-255.
        ...
        for _ in range(cycles * repeat // 10):  # Scale cycles to match 100,000 total iterations
            for i in range(len(transformed)):
    ...
    def reverse_transform_07(self, data, repeat=100000):
        """
    ...
    def transform_08(self, data, repeat=100):
        """
        Transformation using 3 base-10 digits of pi with circular shift and prime-based pre-transformation.
        """
        transformed = bytearray(data)
        pi_length = len(self.PI_DIGITS)
        data_size_kb = len(data) / 1024
    ...
    def reverse_transform_08(self, data, repeat=100000):
        """
    ...
    def transform_09(self, data, repeat=100):
        """
        New transformation using 3 base-10 digits of pi with prime-based XOR and seed table modulation.
        Applies 100 iterations for enhanced diffusion.
        """
        transformed = bytearray(data)
        pi_length = len(self.PI_DIGITS)
        data_size_kb = len(data) / 1024
        ...
        # Circular shift pi digits based on file size
        shift = len(data) % pi_length
        self.PI_DIGITS = self.PI_DIGITS[shift:] + self.PI_DIGITS[:shift]

        # Pre-transformation: XOR with nearest prime and seed table value
        size_prime = find_nearest_prime_around(len(data) % 256)
    ...
    def reverse_transform_09(self, data, repeat=100000):
        """
        Reverse transformation for transform_09 (self-inverse since XOR is its own inverse).
        """
        transformed = bytearray(data)
        pi_length = len(self.PI_DIGITS)
        data_size_kb = len(data) / 1024
    ...
    def compress_with_best_method(self, data, filetype):
        ...
        transformations = [
            ...
            (7, self.transform_07),
            (8, self.transform_08),
            (9, self.transform_09),  # Added new transformation
        ]
        # Prioritize transform_07, transform_08, and transform_09 for JPEG and TEXT
    ...
    def decompress_with_best_method(self, data):
        ...
        reverse_transforms = {
            ...
            9: self.reverse_transform_09,  # Added reverse transformation
        }
...
def main():
    ...
    if choice == '1':
        ...
        try:
            ...
            # Preserve original extension based on marker
            orig_ext = os.path.splitext(input_file)[1].lower()
            if marker in [7, 8, 9] and not output_file.endswith(('.jpg', '.jpeg', '.txt')):
